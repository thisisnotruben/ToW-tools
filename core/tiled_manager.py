...
class Tiled:
    ...
    def _get_center_pos(pos=(), size=()):
        ...
        x = int(
            round(
                (int(pos[0]) + int(size[0]) / 2.0) / float(Tiled.cell_size)) *
            float(Tiled.cell_size))
        x += offset
        y = int(pos[1]) + offset
        return (int(x), int(y))
        ...
        for child in root:
            if child.tag == "group" and child.attrib["name"] == "zed":
                for child in root[group_index]:
                    if child.tag == "objectgroup" and child.attrib[
                            "name"] == "characters":
                        break
                    layer_index += 1
                break
    ...
    def _get_character_attributes(self):
        ...
        for child in root[group_index][layer_index]:
            ...
            id_idx = 0
            for child in root:
                if "id" in child.attrib and int(
                        child.attrib["id"]) == ID - tile_id:
        ...
        for child in ET.parse(self.tiled["map_file"]).getroot():
            if child.tag == "group" and child.attrib["name"] == "meta":
                for sub_child in child:
                    if sub_child.tag == "objectgroup" and sub_child.attrib[
                            "name"] == "paths":
                        for path in sub_child:
                            if not "name" in path.attrib or not path.attrib[
                                    "name"].isnumeric():
                                print(
                                    "--> NO NAME SET IN PATH-ID: %s\n--> NAME NEEDS TO BE UNIT-ID\n--> ABORTING"
                                    % path.attrib["id"])
                                exit(1)
                            elif not path.attrib["name"] in unit_spawn_locs:
                                print(
                                    "--> PATH NAME: %s DOESN'T MATCH WITH ANY CHARACTER-ID\n--> ABORTING"
                                    % path.attrib["name"])
                                exit(1)
                            ...
                            for p in parse_points:
                                p = p.split(",")
                                p = (int(p[0]) + point_origin[0],
                                     int(p[1]) + point_origin[1])
                                points.append(p)
    ...
    def _get_unit_spawn_locs(self):
        spawn_locs = {}
        group_index, layer_index = self._get_map_character_layer()
        for child in ET.parse(
                self.tiled["map_file"]).getroot()[group_index][layer_index]:
            unit_atts = child.attrib
            if "width" in unit_atts:
                spawn_loc = Tiled._get_center_pos(
                    (unit_atts["x"], unit_atts["y"]),
                    (unit_atts["width"], unit_atts["height"]))
                spawn_locs[unit_atts["id"]] = spawn_loc
        return spawn_locs

    def is_debugging(self):
        return os.path.exists(
            os.path.join(self.tiled["map_dir"], Tiled.temp_dir))

    def debug_map(self):
        os.chdir(self.tiled["map_dir"])
        debug = True
        if os.path.exists(Tiled.temp_dir):
            Tiled._debug_map_move_files(Tiled.temp_dir,
                                        self.tiled["tileset_dir"])
            shutil.rmtree(Tiled.temp_dir)
            debug = False
        else:
            os.mkdir(Tiled.temp_dir)
            Tiled._debug_map_move_files(self.tiled["tileset_dir"],
                                        Tiled.temp_dir)
            Tiled._debug_map_move_files(self.tiled["debug_dir"],
                                        self.tiled["tileset_dir"])
            with open(os.path.join(Tiled.temp_dir, "README.txt"), "w") as f:
                f.write(
                    "Don't delete folder or contents of folder manually. If you do, you've done messed up."
                )
        print(
            "--> MAP DEBUG OVERLAY: %s\n--> PRESS (CTRL-T) TO REFRESH IF HASN'T SHOWN"
            % debug)
        ...
        for tileset in os.listdir(self.tiled["tileset_dir"]):
            if tileset.endswith(Tiled.img_ext):
                ...
                if not self.debug[tileset] in defined_color_names:
                    print(
                        "--> COLOR: (%s) NOT DEFINED FOR TILESET: (%s)\n--> DEFINED COLORS:"
                        % (self.debug[tileset], tileset))
    ...
    def make_32_tilesets(self):
        ...
        for tileset in os.listdir(self.tiled["tileset_dir"]):
            if tileset in self.tilesets_32:
                # make paths
                src = os.path.join(self.tiled["tileset_dir"], tileset)
                new_file_name = "%s_32%s" % (os.path.splitext(tileset)[0],
                                             Tiled.img_ext)
    ...
    def export_map(self):
        ...
        for template in os.listdir(self.tiled["template_dir"]):
            if "target_dummy" in template:
                tD_root = ET.parse(
                    os.path.join(self.tiled["template_dir"],
                                 template)).getroot()
                for child in tD_root:
                    if child.tag == "object":
                        tD_size = (int(child.attrib["width"]),
                                   int(child.attrib["height"]))
        if bool(tD_size):
            for group in root:
                if group.tag == "group" and group.attrib["name"] == "zed":
                    for layer in group:
                        if layer.tag == "objectgroup" and layer.attrib[
                                "name"] == "target_dummys":
                            for td in layer:
                                td_atts = td.attrib
                                spawn_loc = Tiled._get_center_pos(
                                    (td_atts["x"], td_atts["y"]), tD_size)
        ...
        for tag_name in ["light", "grave"]:
            for group in root:
                if group.tag == "group" and group.attrib["name"] == "meta":
                    for layer in group:
                        if layer.tag == "objectgroup" and tag_name in layer.attrib[
                                "name"]:
                            for thing in layer:
                                thing.set("name",
